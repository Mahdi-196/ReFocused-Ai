#!/usr/bin/env python3
"""
10-Minute Cloud Test for Single-Threaded Tokenization
...
class CloudTestTokenizer:
    """10-minute test tokenizer for cloud validation."""
    ...
    def __init__(
        ...
        self.logger.info(f"🧪 10-MINUTE CLOUD TEST TOKENIZER")
    ...
    def setup_logging(self):
        ...
        log_file = f"cloud_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
    ...
    def run_test(self):
        ...
        self.logger.info(f"")
        self.logger.info(f"🎯 === 10-MINUTE TEST RESULTS ===")
        ...
        # Save test summary
        summary = {
            "test_duration_minutes": total_elapsed / 60,
            ...
            "estimated_full_time_hours": estimated_full_time_hours if total_sequences > 0 and files_processed > 0 else None,
            "estimated_total_sequences": estimated_total_sequences if total_sequences > 0 and files_processed > 0 else None,
        ...
        self.logger.info(f"📋 Test summary saved to {self.output_dir}/test_summary.json")
...
def main():
    """Main function for 10-minute cloud test."""
    print("=" * 60)
    print("🧪 10-MINUTE CLOUD TOKENIZATION TEST")
    print("=" * 60)
    print("This will test the single-threaded tokenization for 10 minutes")
    print("to validate it works properly in your cloud environment.")
    print()
    print("What this test will do:")
    print("• Process files for exactly 10 minutes")
    print("• Save partial results to test_cloud_tokenized/")
    print("• Calculate processing rates and projections")
    print("• Validate the fix works before full run")
    print()
    print("🔧 Mode: Single-threaded (no multiprocessing issues)")
    print("📁 Input: data/cleaned/*.jsonl")
    print("📤 Output: test_cloud_tokenized/")
    print()
    
    try:
        # Create test tokenizer
        tokenizer = CloudTestTokenizer(
            tokenizer_path="models/tokenizer/transformers_tokenizer",
            input_dir="data/cleaned",
            output_dir="test_cloud_tokenized",
            max_length=1024,
            stride=512,
            batch_size=500,
            max_test_minutes=10
        ...
        print("\n" + "=" * 60)
        print("🎉 TEST COMPLETE!")
        print("Check the logs and test_cloud_tokenized/ directory for results.")
        print("If test passed, you can run the full tokenization with confidence.")
        
    except Exception as e:
        print(f"❌ Test failed to start: {e}")
        print("Check that:")
        print("• models/tokenizer/transformers_tokenizer/tokenizer.json exists")
        print("• data/cleaned/ directory contains .jsonl files")
